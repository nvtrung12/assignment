...
function showUploadedFile() {
	...
	if (js['realName']) {
		if (js['realName']['cbookFile'])
			$('#cbookUploaded').html(js['realName']['cbookFile']);
	}
}
...
$(document).ready(function() {
	...
	// concept)
	uploadFileAjax("#cbookFile", fn('cbookFile'));

	$("#f1").submit(function(e) {
		console.log($(this).serialize());
		queryGraph1('uploads/' + js['cbookFile']);
		return false;
	});
